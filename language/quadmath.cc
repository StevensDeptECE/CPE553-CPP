...
void benchmark(const char msg[], uint64_t n, Func func, T start, T end) {
	clock_t t0 = clock();
	func(start, end);
	clock_t t1 = clock();
	cout << msg << ": elapsed=" <<  (t1-t0)*1e-6 << endl;
}

#define bench(T, n, func)											\
	benchmark(#T, n, func<T>, T(1), T(n))
...
int main()
{
	...
	constexpr uint64_t n = 100000000; // 100 mill
	bench(float, n, sum);
	//	bench(double, n, sum);
	//	bench(__float128, n, sum);
	//	bench(__int128, n, sum);
  return 0;
}
